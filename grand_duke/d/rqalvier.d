...
IF ~InMyArea("rqbelt")~ THEN BEGIN BossISHere
    SAY ~<CHARNAME> pleasure it is meet you though you should speaking to Duke Belt.~
    IF ~~ THEN EXTERN ~rqbelt~ DukeBeltIntro
END

IF ~!InMyArea("rqbelt")~ THEN BEGIN AlvierIntro
    SAY ~My name is Alvier, and I welcome you to the Ducal Palace. Now with introductions out of the way, lets talk about Duke Belt's offer to mend relations shall we?~
    IF ~~ THEN REPLY ~Fine, say what you need to say.~ GOTO MoveToImportant
    IF ~~ THEN REPLY ~Why is there such hostility between Belt and Krivy?~ GOTO WhyChose
    IF ~~ THEN REPLY ~I do not think I have anything to say to you, Alvier. Farewell.~ GOTO RejectionOfAlvier
END

IF ~~ THEN BEGIN MoveToImportant
    SAY ~Duke Belt offers you a choice: either continue to support Krivy, or become an envoy for calming down tensions between Amn and us. As Hero of Baldur's Gate, you will be taken seriously by the Council of Five.~
    IF ~~ THEN REPLY ~Why do I have to choose?~ GOTO WhyChose
    IF ~~ THEN REPLY ~I will go to Amn for the sake of the city.~ GOTO FineThen
    IF ~~ THEN REPLY ~No, I do not want to do that. I still have some unfinished work to do.~ GOTO RejectionOfAlvier
END

IF ~~ THEN BEGIN WhyChose
    SAY ~I am poorly instructed to speak to you about this subject. Please speak to me again if you wish to work for Duke Belt.~
    IF ~~ THEN EXIT
END

IF ~~ THEN BEGIN FineThen
    SAY ~Duke Belt is delighted to hear that. We can use magic to move you immediately please express your readiness to me if you wish to do so.~
    IF ~~ THEN REPLY ~Yes i am ready.~ DO
...
IF ~~ THEN BEGIN RejectionOfAlvier
    SAY ~Unfortunate. Duke Belt's offer is time limited, so if you change your mind later and I won't be here, his offer will have expired.~
    IF ~~ THEN EXIT
END
